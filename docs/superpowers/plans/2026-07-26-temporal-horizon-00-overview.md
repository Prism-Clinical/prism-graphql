...
**Status:** 01, 02, 03, 05 merged to `main`. **04 executed** on `feat/temporal-horizon-evaluator-kernel` (10 tasks, D1–D10, suite 958→1236 passing with the 9 pre-existing scorer failures unchanged; no assertion that existed on `main` was removed or weakened). 04b, 05b, 06, 07, 08, 09 outstanding.
...
| 04 | Evaluator via kernel (§4,§8-kernel) — **reachability moved to 07** | `temporal/condition-adapter.ts`, `temporal/gate-policy.ts`; edits to `gate-evaluator.ts`, `select-facts.ts`, `resolvers/helpers/resolution-context.ts`, `import/validator.ts`, both engines, both start mutations, `types/index.ts`, `index.ts` |
...
### Plan 04 — **EXECUTED 2026-08-11/12.** Produces `AdaptedCondition` + `adaptCodedCondition` / `adaptAttributeCondition` / `toFactSelectionCondition` / `parseConditionOverride` / `nodeOverrideFor` / `attributeNamespaceToField` / `codedVitalsSystemError` (`temporal/condition-adapter.ts`); `effectivePolicyFor` (`temporal/gate-policy.ts`) — the single place the policy version is read; a **version seam** in `evaluateGate(gate, deps)` with `GateEvaluationDeps`, an exported `CONDITION_EVALUATORS` table, `evaluateConditionLegacy` (today's code, byte-for-byte) and `evaluateConditionKernel`; `indeterminate`/`uncertainty` on `GateEvaluationResult`; `factStoreForInput`/`factStoreForSession` and `resolveTemporalPolicyVersion(ctx: DataSourceContext)`; `temporalPolicyVersion?: string` on `DataSourceContext`. Exports `sweepableConditions` and adds `horizon`/`status` to `CodedCondition` and to the import validator's key allowlists. Consumes 01–03 and 05.

**Ten decisions settled during execution — D1–D10 in the plan.** The load-bearing ones for later plans: **D7** undated observations are admitted but not orderable (scalar/series fail closed); **D8** the aggregate class selects on the start bound, not overlap, and a vitals `count_in_window` is consequently always zero; **D9** a coded `vitals` condition may not carry a `system`; **D10** both context doors run the same trust parsing.

**Reachability is UNCHANGED by plan 04 and moves to plan 07.** It has neither the inputs nor a fact source until 07's snapshot mapper — `matchedPathways(patientId)` runs before a session exists, with no temporal arguments. `ALWAYS_EVALUABLE` therefore retains exactly its current meaning. Four questions plan 07 must answer: where its **request-scoped clock** comes from; which **policy version** an advisory read uses; how **`temporal_defaults`** are loaded outside a session; and the **snapshot→`NormalizedFact`** mapping itself.
...
### Plan 04b — `satisfaction_check.lookback_days` (`prerequisites.ts`), the second temporal filter no cascade level governs. Split out of 04 (D4) and still unowned. It is a *different evaluator* with a different shape (`{type, code, system, lookback_days}` and no `field`), which is why routing it through the kernel was not folded into 04.

**Correction to plan 01's published claim.** `select-facts.ts` asserted that its candidate rules mirroring the current evaluator made `legacy-v0` "genuinely behavior-preserving". They do not: candidate parity is one axis of four, and record validity, ordering and vitals bucketing all differ by design. What `legacy-v0` preservation actually rests on is plan 04's **version seam** — it runs the untouched legacy function and never reaches the kernel. Corrected in the source comment during 04 Task 10.
