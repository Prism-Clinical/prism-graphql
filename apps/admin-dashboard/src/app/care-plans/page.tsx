      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage reusable care plans with predefined goals and interventions
            {totalCount > 0 && ` (${totalCount} care plans)`}
          </p>
        </div>
        ...
        <CardBody>
          {loading ? (
            ...
            <div className="text-center py-12 text-red-600">
              <p>Error loading care plans. Please try again.</p>
            </div>
          ) : templates.length === 0 ? (
            <EmptyState
                  ...
                  <TableRow key={template.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        <Link href={`/care-plans/${template.id}`}>
                          <Button variant="ghost" size="sm">View</Button>
                        </Link>
                        <Link href={`/care-plans/${template.id}/edit`}>
