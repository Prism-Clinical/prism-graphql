/**
 * A patient-context change re-evaluates the DecisionPoint that reads it.
 *
 * Branch confidences are computed AT the DecisionPoint but were recorded
 * against each branch TARGET. `addPatientContext` seeds the affected targets
 * and an incremental resolve walks DOWNSTREAM only, so it never revisited the
 * parent that decides which branches qualify — new context could move a branch
 * across the threshold and the fork would keep its old answer.
 *
...
describe('a DecisionPoint depends on the context its branches are scored from', () => {
  it('records the branch scorer inputs against the DecisionPoint itself', async () => {
    mockBuild.mockResolvedValue(rctx());
    await resolutionMutations.startResolution(
      null as never,
      {
    ...
    const created = mockedCreateSession.mock.calls[0][1] as unknown as {
      dependencyMap: { scorerInputs: Map<string, Set<string>> };
    };

    // Without this the fork is unreachable from a context change: seeding its
    // branch targets walks downstream, away from the node that decides.
    expect(created.dependencyMap.scorerInputs.has('dp-1')).toBe(true);
  });
});
