...
import { RecommendationItemService, initializeDatabase } from '@recommendation-items/services/database';
...
describe('RecommendationItemService', () => {
  let service: RecommendationItemService;
  ...
  beforeEach(() => {
    ...
    service = new RecommendationItemService();
  });
  ...
  describe('getAllRecommendationItems', () => {
    ...
    it('fetches recommendation items from database if not cached', async () => {
      ...
      expect(result[0].type).toBe(RecommendationItemType.Medication);
    });
  });
  ...
  describe('getRecommendationItemsByType', () => {
    it('fetches recommendation items by type from database', async () => {
      ...
      const result = await service.getRecommendationItemsByType(RecommendationItemType.Medication);
  ...
  describe('getRecommendationItemsByEvidenceLevel', () => {
    it('fetches recommendation items by evidence level from database', async () => {
      ...
      const result = await service.getRecommendationItemsByEvidenceLevel(EvidenceLevel.LevelI);
      ...
      expect(result).toHaveLength(1);
      expect(result[0].evidenceLevel).toBe(EvidenceLevel.LevelI);
    });
  });
  ...
  describe('createRecommendationItem', () => {
    it('creates a new recommendation item', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO recommendation_items'),
        [
          'MEDICATION',
          'Lisinopril 10mg',
      ...
      expect(result.type).toBe(RecommendationItemType.Medication);
  ...
  describe('updateRecommendationItem', () => {
    it('updates an existing recommendation item', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE recommendation_items'),
        ['Metformin 1000mg', 'Updated dosage for type 2 diabetes', 'Take once daily with dinner', '1']
      );
