/**
 * Care Plan Validator Client for Safety Service
 *
 * HTTP client for the Care Plan Validator ML service.
 * Used to detect anomalies and validate safety of recommendations.
 */
...
/**
 * Safety-specific validation result with alert information
 */
export interface SafetyValidationResult extends ValidationResult {
  alertLevel: "NONE" | "LOW" | "MEDIUM" | "HIGH" | "CRITICAL";
  alertMessage?: string;
  requiresReview: boolean;
  reviewPriority?: "P0_CRITICAL" | "P1_HIGH" | "P2_MEDIUM" | "P3_LOW";
}
...
export class ValidatorClient {
  ...
  /**
   * Perform safety-specific validation with alert classification.
   * This method wraps validateRecommendation and adds safety-specific logic.
   */
  async validateForSafety(
    input: ValidationInput
  ): Promise<SafetyValidationResult> {
    const result = await this.validateRecommendation(input);

    // Determine alert level based on anomaly score and validation tier
    let alertLevel: SafetyValidationResult["alertLevel"] = "NONE";
    let alertMessage: string | undefined;
    let requiresReview = false;
    let reviewPriority: SafetyValidationResult["reviewPriority"];

    if (result.isAnomaly) {
      if (result.anomalyScore >= 0.9) {
        alertLevel = "CRITICAL";
        alertMessage = `Critical anomaly detected: Recommendation significantly deviates from known patterns. Anomaly score: ${result.anomalyScore.toFixed(2)}`;
        requiresReview = true;
        reviewPriority = "P0_CRITICAL";
      } else if (result.anomalyScore >= 0.8) {
        alertLevel = "HIGH";
        alertMessage = `High anomaly detected: ${result.deviationFactors.join(", ") || "Unusual recommendation pattern"}`;
        requiresReview = true;
        reviewPriority = "P1_HIGH";
      } else if (result.anomalyScore >= 0.6) {
        alertLevel = "MEDIUM";
        alertMessage = `Moderate anomaly: ${result.deviationFactors.join(", ") || "Pattern deviation detected"}`;
        requiresReview = true;
        reviewPriority = "P2_MEDIUM";
      } else {
        alertLevel = "LOW";
        alertMessage = "Minor pattern deviation detected";
        requiresReview = false;
      }
    } else if (result.validationTier === "BLOCKED") {
      alertLevel = "HIGH";
      alertMessage = `Recommendation blocked: ${result.deviationFactors.join(", ") || "Failed validation"}`;
      requiresReview = true;
      reviewPriority = "P1_HIGH";
    } else if (result.validationTier === "NEEDS_REVIEW") {
      alertLevel = "MEDIUM";
      alertMessage = `Recommendation needs review: Confidence score ${result.confidenceScore.toFixed(2)}`;
      requiresReview = true;
      reviewPriority = "P2_MEDIUM";
    }
  ...
  /**
   * Batch safety validation with alert classification.
   */
  async validateBatchForSafety(
    inputs: ValidationInput[]
  ): Promise<SafetyValidationResult[]> {
    const batchResult = await this.validateBatch(inputs);
    ...
    return batchResult.results.map((result) => {
      ...
      if (result.isAnomaly) {
        if (result.anomalyScore >= 0.9) {
          alertLevel = "CRITICAL";
          alertMessage = `Critical anomaly detected: Anomaly score ${result.anomalyScore.toFixed(2)}`;
          requiresReview = true;
          reviewPriority = "P0_CRITICAL";
        } else if (result.anomalyScore >= 0.8) {
          alertLevel = "HIGH";
          alertMessage = `High anomaly: ${result.deviationFactors.join(", ") || "Unusual pattern"}`;
          requiresReview = true;
          reviewPriority = "P1_HIGH";
        } else if (result.anomalyScore >= 0.6) {
          alertLevel = "MEDIUM";
          alertMessage = "Moderate pattern deviation";
          requiresReview = true;
          reviewPriority = "P2_MEDIUM";
        } else {
          alertLevel = "LOW";
        ...
        alertLevel = "HIGH";
        alertMessage = "Recommendation blocked by validator";
        requiresReview = true;
        reviewPriority = "P1_HIGH";
      } else if (result.validationTier === "NEEDS_REVIEW") {
        alertLevel = "MEDIUM";
