...
import { 
  ...
  InstitutionType,
  initializeDatabase
} from '@institutions/services/database';
...
describe("Institutions Database Service", () => {
  ...
  describe("Institution Service", () => {
    describe("createInstitution", () => {
      it("creates a new institution in database", async () => {
        const institutionData = {
          ...
          type: InstitutionType.HOSPITAL_SYSTEM,
          address: {
        ...
        const result = await institutionService.createInstitution(institutionData);

        expect(result).toBeDefined();
        expect(result.id).toBeDefined();
        expect(result.name).toBe("Metropolitan Health System");
        expect(result.type).toBe(InstitutionType.HOSPITAL_SYSTEM);
      ...
      it("creates institution without optional fields", async () => {
        const institutionData = {
          ...
          type: InstitutionType.MEDICAL_CENTER,
          address: {
        ...
        expect(result.type).toBe(InstitutionType.MEDICAL_CENTER);
      ...
      it("throws error for duplicate institution name", async () => {
        ...
        await expect(institutionService.createInstitution(institutionData))
    ...
    describe("getInstitutionById", () => {
      it("retrieves institution by ID", async () => {
        ...
        const result = await institutionService.getInstitutionById(testInstitution.id);
      ...
      it("returns null for non-existent institution", async () => {
        const result = await institutionService.getInstitutionById("non-existent-id");
      ...
      it("uses Redis cache for subsequent requests", async () => {
        ...
        const result1 = await institutionService.getInstitutionById(testInstitution.id);
        ...
        const result2 = await institutionService.getInstitutionById(testInstitution.id);
    ...
    describe("getInstitutions", () => {
      ...
      it("filters by institution type", async () => {
        ...
        const result = await institutionService.getInstitutions({ type: InstitutionType.HOSPITAL_SYSTEM });

        expect(result.length).toBe(2);
        expect(result.every(i => i.type === InstitutionType.HOSPITAL_SYSTEM)).toBe(true);
      });
      ...
      it("respects limit and offset parameters", async () => {
        ...
        const result = await institutionService.getInstitutions({ limit: 2, offset: 1 });

        expect(result.length).toBe(2);
      });

      it("uses Redis cache for repeated queries", async () => {
        await testHelpers.insertInstitution(pool, { name: "Cache Test", type: "CLINIC_NETWORK" });

        await institutionService.getInstitutions({ limit: 10, offset: 0 });
    ...
    describe("updateInstitution", () => {
      it("updates institution information", async () => {
        ...
        const result = await institutionService.updateInstitution(testInstitution.id, updates);
      ...
      it("returns null for non-existent institution", async () => {
        const result = await institutionService.updateInstitution("non-existent", {
          name: "Updated"
        });
      ...
      it("invalidates cache after update", async () => {
        ...
        // Get institution to cache it
        await institutionService.getInstitutionById(testInstitution.id);

        // Update institution
        await institutionService.updateInstitution(testInstitution.id, {
    ...
    describe("deleteInstitution", () => {
      it("deletes institution from database", async () => {
        ...
        const result = await institutionService.deleteInstitution(testInstitution.id);
        expect(result).toBe(true);

        const deletedInstitution = await institutionService.getInstitutionById(testInstitution.id);
        expect(deletedInstitution).toBeNull();
      });

      it("returns false for non-existent institution", async () => {
        const result = await institutionService.deleteInstitution("non-existent");
      ...
      it("handles cascade deletion of related hospitals", async () => {
        ...
        // This test verifies that foreign key constraints are properly handled
        const result = await institutionService.deleteInstitution(testInstitution.id);
        expect(result).toBe(true);
      });
    });
  ...
  describe("Hospital Service", () => {
    describe("createHospital", () => {
      ...
      it("throws error for invalid institution reference", async () => {
        ...
        await expect(hospitalService.createHospital(hospitalData))
    ...
    describe("getHospitalById", () => {
      it("retrieves hospital by ID", async () => {
        ...
        // Note: We'll need to add hospital insertion helper or use institutions table
        ...
        const createdHospital = await hospitalService.createHospital(hospitalData);
        const result = await hospitalService.getHospitalById(createdHospital.id);
      ...
      it("returns null for non-existent hospital", async () => {
        const result = await hospitalService.getHospitalById("non-existent-id");
        expect(result).toBeNull();
      });
      ...
      it("uses Redis cache for subsequent requests", async () => {
        ...
        // First request - should cache the result
        const result1 = await hospitalService.getHospitalById(createdHospital.id);
        expect(result1).toBeDefined();
        ...
        // Second request - should use cache
        const result2 = await hospitalService.getHospitalById(createdHospital.id);
        expect(result2).toBeDefined();
    ...
    describe("getHospitalsByInstitution", () => {
      it("retrieves hospitals for a specific institution", async () => {
        const testInstitution = await testHelpers.insertInstitution(pool, {});
        const otherInstitution = await testHelpers.insertInstitution(pool, {});
        ...
        // Create hospital for different institution
        await hospitalService.createHospital({
          name: "Other Hospital",
        ...
        const result = await hospitalService.getHospitalsByInstitution(testInstitution.id);
    ...
    describe("updateHospital", () => {
      it("updates hospital information", async () => {
        ...
        const result = await hospitalService.updateHospital(hospital.id, updates);
      ...
      it("returns null for non-existent hospital", async () => {
        const result = await hospitalService.updateHospital("non-existent", {
          name: "Updated"
        });
    ...
    describe("deleteHospital", () => {
      it("deletes hospital from database", async () => {
        ...
        const result = await hospitalService.deleteHospital(hospital.id);
        expect(result).toBe(true);

        const deletedHospital = await hospitalService.getHospitalById(hospital.id);
        expect(deletedHospital).toBeNull();
      });

      it("returns false for non-existent hospital", async () => {
        const result = await hospitalService.deleteHospital("non-existent");
        expect(result).toBe(false);
      });
    });
  });
  ...
  describe("Error handling", () => {
    ...
    it("handles invalid UUID format", async () => {
      const result = await institutionService.getInstitutionById("invalid-uuid");
      expect(result).toBeNull();
    });
  });
});