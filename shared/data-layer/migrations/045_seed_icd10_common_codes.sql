-- Migration 045: Seed common ICD-10-CM codes
--
-- Loads ~436 frequently-used ICD-10 codes covering the major chapters seen
-- in primary care, derived from CMS public-domain code set. Mirrors the
-- standalone seed at shared/data-layer/seed/icd10-common-codes.sql; the
-- standalone file is preserved for tools that load seed manually.
--
-- This was previously a manually-applied seed file; it is now a migration so
-- `make migrate` brings up a fresh install end-to-end without manual steps.
--
-- Idempotent: every INSERT uses ON CONFLICT (code) DO NOTHING.
--
-- Compat with environments where the older monolithic 044 already added a
-- NOT NULL constraint on icd10_codes.path: we DROP NOT NULL up front (no-op
-- if not present), insert, and migration 046 adds NOT NULL back after the
-- backfill is complete.

ALTER TABLE icd10_codes ALTER COLUMN path DROP NOT NULL;
